...
static void common_drive(int index, tCarElt* car, tSituation *s)
{
		...
		((cmd[CMD_SCC].type == GFCTRL_TYPE_SKEYBOARD) && skeyInfo[cmd[CMD_SCC].val].edgeUp))
	{
		...
		flag_data[CC] = (short)HCtx[idx]->ParamScc;
		if (flag_data[CC] > 0)
		{
			...
			// for experiments
			sensing_data[TARGET_WHEEL_SPEED] = sensing_data[TARGET_WHEEL_SPEED] * 80.0 / sensing_data[TARGET_SPEED];
			sensing_data[TARGET_SPEED] = 100.0;
		}
	}

	if (actuating_data[NUM_DUMMY] == 10000 && cnt2 == 0)  // This is for HyunJun's Essay. It makes LKAS turn on after speed = 100. 
	{
		HCtx[idx]->ParamScc = 1 - HCtx[idx]->ParamScc;
		sensing_data[CC_TRIGGER] = (float)HCtx[idx]->ParamScc;
		cnt2++;
	}
	...
	//LKAS
	if (actuating_data[STEER_MODE] < 0.5)	// calculate STEER_VALUE
	{
		...
		if (car->_trkPos.seg->type == TR_STR)
			...
			length = (car->_trkPos.seg->arc - car->_trkPos.toStart)*car->_trkPos.seg->radius;
	...
	else
	{
		...
		track_info = RtTrackSideTgAngleL(&(car->_trkPos)) - car->_yaw;
		NORM_PI_PI(track_info);
	}
	...
	if ((((PI / 2) <= car->_yaw) && (car->_yaw <= PI)) && ((-PI <= direction_road) && (direction_road <= -PI / 2)))
	{
	...
	float lateral = 0.0;
	float middle = (car->_trkPos.toLeft + car->_trkPos.toMiddle) / 2.0;

	if (car->_trkPos.toLeft >= car->_trkPos.toMiddle)
	{
		lateral = (middle - car->_trkPos.toMiddle)*3.5 / 10.0;
	}

	else
	{
		lateral = -((middle - car->_trkPos.toLeft)*3.5 / 10.0);
	}


	sensing_data[LATERAL] = lateral;

	float speed_error = 0.0;

	speed_error = (car->_speed_x * 3.6) - sensing_data[TARGET_SPEED];

	sensing_data[SPEED_ERROR] = speed_error;


	/*************/

	// Cruise Control
	float gr2[8] = { -2.0, 10.0, 2.9, 2.3, 1.87, 1.68, 1.54, 1.46 };
	float pi2 = 2.0*3.1415;
	float rm = car->_enginerpmRedLine;
	switch (HCtx[idx]->drivetrain)
	{
	case DRWD:
		sensing_data[WHEEL_SPEED] = car->_wheelSpinVel(REAR_RGT);
		break;
	case DFWD:
		sensing_data[WHEEL_SPEED] = car->_wheelSpinVel(FRNT_RGT);
		break;
	case D4WD:
		sensing_data[WHEEL_SPEED] = (car->_wheelSpinVel(REAR_RGT) + car->_wheelSpinVel(FRNT_RGT)) / 2;
		break;
	default:
		break;
	}
	...
	int count = 1;
	float re = sensing_data[TARGET_WHEEL_SPEED] / pi2 * 6.0 * gr2[car->_gear] / rm;
	if (count == 1)
	{

		sensing_data[ACCEL_VALUE] = re;
		count++;
	}

	// no Avees
	sensing_data[SPEED] = 3.6*car->_speed_x;
	//      sensing_data[TARGET_SPEED] = 100.0;		already assign
	sensing_data[ACCEL_VALUE] = actuating_data[ACCEL];
	sensing_data[STEER_ANGLE] = direction_road;

	// Avees
	sensing_data[LOOKAHEAD] = car->_yaw;
	sensing_data[TO_LEFT] = car->_trkPos.toLeft;
	sensing_data[TO_RIGHT] = car->_trkPos.toRight;
	sensing_data[TO_MIDDLE] = car->_trkPos.toMiddle;
	sensing_data[WIDTH] = car->_trkPos.seg->width;
	sensing_data[FUEL] = car->_fuel;
	sensing_data[ENGINE_RPM] = car->_enginerpm;
	sensing_data[GEAR_RATIO] = car->_gearRatio[car->_gearNb + car->_gearOffset];
	sensing_data[GEAR] = car->_gear;
	sensing_data[PITCH] = car->_pitch;
	sensing_data[ROLL] = car->_roll;

	// no Avees
	sensing_data[YAW_RATE] = car->_yaw_rate;
	sensing_data[DISTANCE] = dist;

	// Avees
	sensing_data[POS_X] = car->_pos_X;
	sensing_data[POS_Y] = car->_pos_Y;


	/**** Avees ****
	sensing_data[FUEL] = car->_trkPos.toMiddle;
	sensing_data[GEAR_RATIO] = car->_trkPos.seg->width;
	sensing_data[GEAR] = car->_gear;
	sensing_data[ENGINE_RPM] = car->_trkPos.seg->width;
	sensing_data[PITCH] = car->_pitch;
	sensing_data[ROLL] = car->_roll;
	sensing_data[LOOKAHEAD] = car->_yaw;
	sensing_data[TO_LEFT] = car->_trkPos.toLeft;
	...
	************/
	...
	sprintf(car->_msgCmd[0], "%s %s %s %s", (HCtx[idx]->ParamAbs ? "ABS" : ""), (HCtx[idx]->ParamAsr ? "ASR" : ""), (HCtx[idx]->ParamScc ? "SCC" : ""), (HCtx[idx]->ParamLkas ? "LKAS" : ""));
	...
	case GFCTRL_TYPE_JOY_BUT:
		if (cmd[CMD_LEFTSTEER].type == GFCTRL_TYPE_KEYBOARD) {
			...
			/*** kswe ***/
			if (actuating_data[LEFT_STEER] > 0.01)
			{
				ax0 = actuating_data[LEFT_STEER];
			}
			/************/
		}
		else {
	...
	case GFCTRL_TYPE_JOY_BUT:
		...
		if (cmd[CMD_RIGHTSTEER].type == GFCTRL_TYPE_KEYBOARD) {
			...
			/*** kswe ***/
			if (actuating_data[RIGHT_STEER] > 0.01)
			{
				ax0 = actuating_data[RIGHT_STEER];
			}
			/************/
		}
		else {
	...
	//	*** kswe ***
	if (actuating_data[RIGHT_STEER] > 0.01)
		rightSteer = -actuating_data[RIGHT_STEER];
	if (actuating_data[LEFT_STEER] > 0.01)
		leftSteer = actuating_data[LEFT_STEER];
	//	************/


	//***** Avees CHA *****  This part operates Normalization for actuating_data

	if (rightSteer < -0.01)
	{
		float steer_value = rightSteer;
		steer_value = steer_value * car->_steerLock;
		NORM_PI_PI(steer_value);
		steer_value = steer_value / car->_steerLock;
		rightSteer = steer_value;
		leftSteer = 0;
	}
	else if (leftSteer > 0.01)
	{
		float steer_value = leftSteer;
		steer_value = steer_value * car->_steerLock;
		NORM_PI_PI(steer_value);
		steer_value = steer_value / car->_steerLock;
		leftSteer = steer_value;
		rightSteer = 0;
	}

	car->_steerCmd = rightSteer + leftSteer;
	...
	case GFCTRL_TYPE_JOY_BUT:
		...
		/*** kswe ***/
		if (actuating_data[BRAKE] > 0.01)
		{
			car->_brakeCmd = actuating_data[BRAKE];
		}
		/************/
		break;
	...
	case GFCTRL_TYPE_JOY_BUT:
		...
		/*** kswe ***/
		if (actuating_data[ACCEL] > 0.01)
		{
			car->_accelCmd = actuating_data[ACCEL];
		}
		/************/
		break;
	case GFCTRL_TYPE_MOUSE_BUT:
